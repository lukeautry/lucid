...
namespace Lucid.Services
{
	...
	{
		Task<User> GetCurrentUser(string sessionId);
		Task<IEnumerable<SessionUser>> GetSessionUsers();
		Task<IEnumerable<SessionUser>> GetRoomUsers(int roomId);
		Task<Room> GetCurrentRoom(string sessionId);
	}
